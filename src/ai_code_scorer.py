...
#### 5. 大小规范（0-10）
- **0-3**: 存在大量不必要的二进制文件，例如模型，音视频文件，资源文件等, 或者项目文件过大(例如超过1G)，不合理，影响下载和使用。
- **4-6**: 存在一些不必要的二进制文件，管理尚可，但有改进空间。
- **7-8**: 大部分二进制文件管理良好，偶尔有不必要的文件。
- **9-10**: 二进制文件管理非常规范，对于二进制文件有Git LFS 管理，全部必要文件均得到妥善处理。

#### 6. 测试规范（0-10）
- **0-3**: 没有任何的单元测试或者集成测试，也没有任何文档说明 。
- **4-6**: 有默认的mock测试文件，但没有真正代码的测试例子。
- **7-8**: 有一些实际代码的测试例子，但测试覆盖率很低，没有说明怎么进行测试。
- **9-10**: 有较多的代码测试，测试覆盖率高，同时有详细的说明进行如何测试。
...
```json
{
    "评分": {
        "代码质量": {
            "分数": [分数],
            "理由": "[根据 [Code Snapshot] 部分的代码片段进行分析，给出非常详细的理由，用具体的例子进行展开说明，这样对用户才更有帮助，你的回答才更有说服力]"
        },
        "文档规范": {
            "分数": [分数],
